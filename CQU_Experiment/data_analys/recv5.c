...
int port = 10000;
int recv_id;
unsigned int recved_pack[2][PACK_SIZE];
...
void *recv_func()
{
    ...
    while (work != -1)
    {
        ...
        while (work > 0 && work < 15)
            ;
        memset(recved_pack[remark], 0, sizeof(recved_pack) / 2);
        ...
        for (recved_ll = 0; recved_ll < recv_num * 4;)
        {
            ...
            while (ret == 0)
            {
                ret = recv(socket_fd, (char *)recved_pack[remark] + recved_ll, recv_num * 4 - recved_ll, 0);
            }
            ...
            if (ret < 0)
            {
                ...
                write_error_log(&error_fp, recved_pack[remark], 1);
                fclose(error_fp);
                exit(1);
            }
            recved_ll += ret;
...
void sort_data()
{
    ...
    char buf[50] = {'\0'};
    LOCA_TIME ll_adc;
    int remark = sort_count & 1;
    //FILE *fp[8];

    for (int board_count = 0; board_count < recv_num / 1024; board_count++)
    {
        ret = find_board_head(recved_pack[remark] + board_count * 1024, 0, 0);
        if (ret == 100)
        {
        ...
        for (; loca - start_loca < 1023;)
        {
            ...
            ret = find_adc_head(recved_pack[remark] + loca, 0, 0);
            if (ret == 100)
            {
            ...
            _channel = bit_head_read(recved_pack[remark] + loca, 'c');
            ...
            list_adc[_channel][adc_count[_channel]].m_timestamp = bit_time_read(recved_pack[remark] + loca);
            if (list_adc[_channel][adc_count[_channel]].m_timestamp < 0)
            {
                break;
            }
            list_adc[_channel][adc_count[_channel]].m_length = bit_head_read(recved_pack[remark] + loca, 'l');
...
void save_data1()
{
    ...
    for (int c = 0; c < 8; c++)
    {
        ...
        for (int i = 0; i < adc_count[c]; i++)
        {
            ...
            fprintf(log_save, "%d :   channel = %d    timestamp = %llx, loca = %d      %x, %x ,%x, %x, %x, %x, %x\n", i, c + 1, list_adc[c][i].m_timestamp, list_adc[c][i].m_location, recved_pack[remark][list_adc[c][i].m_location - 2], recved_pack[remark][list_adc[c][i].m_location - 1], recved_pack[remark][list_adc[c][i].m_location], recved_pack[remark][list_adc[c][i].m_location + 1], recved_pack[remark][list_adc[c][i].m_location + 2], recved_pack[remark][list_adc[c][i].m_location + 3], recved_pack[remark][list_adc[c][i].m_location + 4]);
        }
        fflush(log_save);
    }
    write_data_error_log(&error_fp, recved_pack[remark], PACK_SIZE, 0);
    fclose(log_save);
    fclose(error_fp);
    ...
    for (addr[7] = 0; addr[7] < adc_count[7]; addr[7]++)
    {
        ...
        if (_channel == -1)
        {
            if (work > 10)
            {
                ...
                for (int c = 0; c < 8; c++)
                {
                    fwrite(recved_pack[remark] + (list_adc[c] + addr[c])->m_location, 4, (list_adc[c] + addr[c])->m_length + 1, log_save);
                }
            }
            else
            {
