...
void helixStep(Float_t step, Float_t *vect, Float_t *vout)//带电粒子在磁场中的径迹，每步长度，初始方向，移动后方向
{
  // extrapolate track in constant field
   Float_t field = 20;      //magnetic field in kilogauss
   enum Evect {kX,kY,kZ,kPX,kPY,kPZ,kPP};//定义枚举Evect类型{0,1,2,3,4,5,6}
   vout[kPP] = vect[kPP];
   ...
   vout[kX]   = vect[kX]  + (f1*vect[kPX] - f2*vect[kPY]);//点的位置
...
void tree2w()
{
   ...
   Gctrak_t gstep;
   t2.Branch("vect",gstep.vect,"vect[7]/F");//数组本身就是地址，不需要&取地址
...
void tree2r()
{
   ...
   static Float_t destep;//静态变量
   TBranch *b_destep = t2->GetBranch("destep");
   b_destep->SetAddress(&destep);//上下两行可以使用如下命令代替
                                 //t2->SetBranchAddress("destep",&destep);
   ...
   for (Long64_t i=0;i<nentries;i++) {
      b_destep->GetEntry(i);//可以不通过branch，可以直接t2->GetEntry(i);来获得数据，更新方式不同
      hdestep->Fill(destep);
   }
