...
public:
  ...
  Det b;
  ClassDef(Event,1)//区分class Event;数字可以改变。？？？
};
...
void tree0() {
  // create a TTree
  TTree *tree = new TTree("tree","treelibrated tree");//声明一个Tree的指针，Tree的name，Tree的Title
  Event *e = new Event;//定义了一个Event的指针e

  // create a branch with energy
  tree->Branch("event",&e);//tree中产生一个Branch，名称为任意(event)，将指针e地址传给tree
  ...
  for (Int_t iev=0;iev<nevent;iev++) {
    ...
    e->a.e=ea;//对a和b探测的能量赋值
    e->b.e=eb;
    e->a.t=gRandom->Rndm();  // random，对时间平分布取均匀随机数
    e->b.t=e->a.t + gRandom->Gaus(0.,.1);  // identical to a.t but a gaussian在a的基础进行晃动取随机数
