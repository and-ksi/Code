...
void tree1w()
{
   ...
   Double_t random;
   Int_t ev;
   t1.Branch("px",&px,"px/F");//在t1的tree中创建Branch，分别命名、地址、以及用
   //   /FDI等规定浮点、双精度、整型  
   ...
   for (Int_t i=0;i<10000;i++) {
     gRandom->Rannor(px,py);//用高斯曲面的方式取出随机数组
     pz = px*px + py*py;
     random = gRandom->Rndm();
     ev = i;
     t1.Fill();
...
void tree1r()
{
   ...
   TTree *t1 = (TTree*)f->Get("t1");//在定义的TFile的指针f中查找名为t1的函数，经过指针变换赋予新的TTree类型t1
   Float_t px, py, pz;
   Double_t random;
   Int_t ev;
   ...
   Long64_t nentries = t1->GetEntries();//取得t1中数据的数量
   for (Long64_t i=0;i<nentries;i++) {
     t1->GetEntry(i);//取得t1中第i组数据，这是以上定义的变量内就有了数据了
     hpx->Fill(px);
     hpxpy->Fill(px,py);
  }
