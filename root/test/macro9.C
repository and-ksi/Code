...
void pull( int n_toys = 10000,//填充1w次
      int n_tot_entries = 100,//每个填充1百次
      int nbins = 40,
      bool do_chi2=true ){//使用likehood还是使用chi fit

    TString method_prefix("Log-Likelihood ");//定义一个TString显示是使用哪一个fit
    if (do_chi2)//如果是chi fit，那么里面内容就显示如下
        method_prefix="#chi^{2} ";//这里是定义了卡方的符号
