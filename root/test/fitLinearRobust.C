...
void fitLinearRobust()
{
   ...
   grr->SetMinimum(-30);//y的最小值
   grr->SetMaximum(80);
   TF1 *ffit1 = new TF1("ffit1", "pol3", -5, 5);//这里应该是设置了空函数，错！
   TF1 *ffit2 = new TF1("ffit2", "pol3", -5, 5);//定义了两个带三次项的多项式，这里不再设置函数形式，直接使用多项式来定义函数
   ffit1->SetLineColor(kBlue);
   ffit2->SetLineColor(kRed);
   TCanvas *myc = new TCanvas("myc", "Linear and robust linear fitting");
   myc->SetGrid();
   grr->Draw("ap");//a是自动，point
   ...
   //will be taken
   grr->Fit(ffit2, "+rob=0.75");//对这些数据取百分之75的有效数据进行拟合，最小值是.5，加号代表直接将线加到上面的图中。
   //
   TLegend *leg = new TLegend(0.6, 0.8, 0.89, 0.89);//加入图例
   leg->AddEntry(ffit1, "Ordinary least squares", "l");//l代表画线
   leg->AddEntry(ffit2, "LTS regression", "l");
   leg->Draw();

   delete [] x;//用这种方式删除动态数组
   delete [] y;
   delete [] e;
