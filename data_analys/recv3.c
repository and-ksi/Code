...
void data_save(){
    ...
    int ret, defference, _length, c;
    unsigned int value;
    int address[8];

    for (int i = 0; i < adc_count[7]; i++){
        c = 7;
        ret = get_latest_data(list_adc[6], list_adc[7][i].m_timestamp, adc_count[6]);
        if(ret >=0){
            address[7] = i;
            address[6] = ret;
            defference = abs(list_adc[6][ret].m_timestamp - list_adc[7][i].m_timestamp);
            if(defference >= 8){
                continue;
            }
            for(c = 5; c > 3; c--){
                ret = get_latest_data(list_adc[c], list_adc[7][i].m_timestamp, adc_count[c]);
                if (ret >= 0 && abs(list_adc[c][ret].m_timestamp - list_adc[7][i].m_timestamp) <= defference + 5)
                {
                    address[c] = ret;
                }
                else
            ...
            if(c == 3){
                ret = get_latest_data(list_adc[c], list_adc[7][i].m_timestamp, adc_count[c]);
                if(ret >= 0){
                    defference = abs(list_adc[3][ret].m_timestamp - list_adc[7][i].m_timestamp);
                    address[c] = ret;
                    if (defference >= 8)
                    {
                        continue;
        ...
        if(c == 0){
            ...
            for(c = 0; c < 8; c++){
                _length += (list_adc[c] + address[c])->m_length;
            }
            value = value | _length;
            fwrite(&value, 4, 1, log_save);
            for(c = 0; c < 8; c++){
                fwrite(recved_pack + (list_adc[c] + address[c])->m_location, 4, (list_adc[c] + address[c])->m_length + 1, log_save);
            }
            fflush(log_save);
        }
    }
